...
export default class Database {
	...
	async newConnection() {
		var db = new sqlite3.Database(path.resolve(path.join(__dirname, '..'), '.innometrics.db'), (err) => {
			if(err){
	...
	async getMetrics() {
		var metrics = [];
		let db = await new sqlite3.Database(path.resolve(path.join(__dirname, '..'), '.innometrics.db'));
