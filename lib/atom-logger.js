...
*/
  async load_configs(){
    if(fs.existsSync(home + '/.VScode-Logger-config.txt')){
      ...
      vscode.window.showErrorMessage("File di configurazione non esistente");
      fs.writeFile(home + '/.VScode-Logger-config.txt',"serverAddress:\n" + "email:\n" + "password:\n" + "protocol: https\n" + "refreshTime: 100\n" + "remember: false\n", function (err) {
        if (err) throw err;
      vscode.window.showInformationMessage("File di configurazione generato nella cartella home");
      });
    };
  },
