...
export default {
  ...
  async StartLogger(){
    ...
    if(this.config.remember === true){ 
      try{
        await this.authentication_routine();
      }catch(err){}
    }
  ...
  async authentication_routine(){
    ...
    await this.server.authenticate(this.config.remember).then(async () => {
      await this.server.getStatistics().then(async (stats)=>{
        await this.dashView.refreshStats(stats);
      }, rejected => {});
  ...
  async load_configs(){
    ...
    let rawConfigJSON = fs.readFileSync(path.resolve(os.homedir() + configDirectory, 'settings.json'), 'utf-8');
    let configJSON = JSON.parse(rawConfigJSON);
      ...
      else if(configJSON.rememberCredentials === true){ vscode.window.showErrorMessage("Inserted credentials format not valid"); }
  },
