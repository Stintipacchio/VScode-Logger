...
export default {
  ...
  async StartLogger(){
    ...
    if(this.auth){ await this.server.authenticate(this.config.remember).then(async () => {
    ...
    else vscode.window.showInformationMessage("Logger started in offline mode");
  },
  ...
  async load_configs(){
    if(fs.existsSync(home + '/.VScode-Logger-config.txt')){
      ...
      if(this.config.serverAddress == '') this.auth = false;
      else this.auth = true;
